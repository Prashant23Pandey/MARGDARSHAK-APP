      ...
      <nav className="fixed top-0 inset-x-0 z-50 bg-void/80 backdrop-blur-2xl border-b border-rule">
        <div className="max-w-[1200px] mx-auto px-4 md:px-8 h-14 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2.5">
            <Logo size={22} />
            <span className="font-serif text-[17px] text-ink tracking-tight">MargDarshak</span>
        ...
        <div className="relative max-w-[1200px] mx-auto px-4 md:px-8 pt-24 md:pt-28 pb-16 md:pb-20 grid grid-cols-1 md:grid-cols-[1fr_480px] gap-8 items-center">
          ...
          <div>
            ...
            <motion.p {...fade(0.35)} className="text-[15px] md:text-[17px] text-ink-secondary leading-relaxed max-w-lg mb-8 md:mb-10">
              MargDarshak is India's first decentralized proof platform. Protect digital truth 
        ...
        <div className="max-w-[1200px] mx-auto px-4 md:px-8">
            ...
            <table className="w-full text-[12px] md:text-[13px] border-collapse">
              <thead>
                <tr className="border-b border-rule text-left">
                  ...
                  <th className="py-3 px-3 md:px-4 text-kesari font-mono text-[10px] md:text-[11px] tracking-wider uppercase font-bold">MargDarshak</th>
                </tr>
              </thead>
            ...
            {[
              {
                ...
                desc: 'Existing tools either do forensics OR timestamping. MargDarshak is the first to combine Error Level Analysis, EXIF metadata extraction, perceptual hashing, AND immutable blockchain anchoring in a single client-side pipeline. You don\'t just prove a file existed — you prove it was unedited, with forensic evidence.',
              ...
              },
              {
                ...
                desc: 'A timestamp proves WHEN. MargDarshak proves WHO held it, WHO co-signed it, and WHO transferred it — with a full audit trail. Co-attestation lets multiple parties vouch for a file. Custody transfer creates a legal-grade provenance chain. Revocation invalidates compromised proofs.',
              ...
              },
              {
                ...
                desc: 'Most platforms say "we delete your files." MargDarshak\'s architecture makes it physically impossible to access them — SHA-256 hashing, ELA, and EXIF extraction all run in browser Web Workers. The server literally never receives raw content. This isn\'t a policy — it\'s physics.',
          ...
          >
            ...
            <p className="text-[13px] md:text-[14px] text-ink-secondary leading-relaxed mt-3">
              That is MargDarshak. That is what nobody else has built.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-[1200px] mx-auto px-4 md:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-px bg-rule">
            {[
              ...
              { icon: Blocks, title: 'Public API', desc: 'REST API v1 with API key auth. Integrate MargDarshak verification and registration into any workflow.' },
            ].map((item, i) => (
              <motion.div
                key={item.title}
                initial={{ opacity: 0, y: 16 }}
      ...
      <footer className="border-t border-rule py-8">
        <div className="max-w-[1200px] mx-auto px-4 md:px-8 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <Logo size={16} />
            <span className="font-serif text-[14px] text-ink-tertiary">MargDarshak</span>
