...
export async function generateCertificate(metadata) {
  ...
  ctx.font = '10px "JetBrains Mono", monospace';
  ctx.fillText('MargDarshak प्रमाण · Bharat Sovereign Proof · MargDarshak.in', 60, H - 25);

  ctx.fillStyle = 'rgba(255,255,255,0.15)';
  ctx.font = '9px "JetBrains Mono", monospace';
  const verifyText = 'Verify at MargDarshak.in/verify · This certificate is cryptographically linked to the Ethereum blockchain';
  ...
  return new Promise((resolve) => {
    canvas.toBlob((blob) => {
      ...
      resolve({ blob, url, filename: `MargDarshak_Certificate_${cleanName}.png` });
    }, 'image/png', 0.95);
  });
}
