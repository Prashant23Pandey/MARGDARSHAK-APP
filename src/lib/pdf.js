...
const QR_BASE = 'https://MargDarshak-app.vercel.app';
...
export function generateCustodyReport(data) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(...GRAY);
    doc.text('MargDarshak — Decentralized Media Authenticator', M, 291);
    doc.text(`Page ${i} of ${pageCount}`, W - M - 20, 291);
    doc.text('https://MargDarshak-app.vercel.app', W / 2, 291, { align: 'center' });
  }
...
export function downloadCustodyReport(data) {
  ...
  a.href = url;
  const filename = data.block?.data?.filename || data.filename || 'MargDarshak-report';
