// MargDarshak Media Verifier — Background Service Worker

const API_BASE = "https://MargDarshakindia-production.up.railway.app/api";
const MAX_HISTORY = 50;

// Create context menu on install
chrome.runtime.onInstalled.addListener(() => {
  chrome.contextMenus.create({
    id: "MargDarshak-verify",
    title: "Verify with MargDarshak",
    contexts: ["image"],
  });
  chrome.contextMenus.create({
    id: "MargDarshak-register",
    title: "Register with MargDarshak",
    contexts: ["image"],
  });
  chrome.contextMenus.create({
    id: "MargDarshak-scan-page",
    title: "MargDarshak: Scan all images on page",
    contexts: ["page"],
  });
});
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === "MargDarshak-scan-page") {
    try {
      await chrome.tabs.sendMessage(tab.id, { type: "MargDarshak_SCAN_ALL" });
    } catch (_) {}
    return;
  }

  if (info.menuItemId === "MargDarshak-register") {
  ...
  if (info.menuItemId !== "MargDarshak-verify") return;
...
async function registerSingleImage(imageUrl, tabId) {
  ...
  try {
    ...
    if (response.status === 409) {
      ...
      try {
        await chrome.tabs.sendMessage(tabId, { type: "MargDarshak_RESULT", ...result });
      } catch (_) {}
      return;
    }
    ...
    console.error("MargDarshak registration failed:", err);
    try {
      await chrome.tabs.sendMessage(tabId, {
        type: "MargDarshak_RESULT",
        status: "error",
        imageUrl,
        error: err.message,
      });
...
async function verifySingleImage(imageUrl, tabId) {
  ...
  try {
    ...
    console.error("MargDarshak verification failed:", err);
    try {
      await chrome.tabs.sendMessage(tabId, {
        type: "MargDarshak_RESULT",
        status: "error",
...
// Call MargDarshak verification API
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === "MargDarshak_VERIFY_URL") {
    (async () => {
      try {
  ...
  if (msg.type === "MargDarshak_VERIFY_IMAGE") {
    (async () => {
      try {
  ...
  if (msg.type === "MargDarshak_SCAN_TAB") {
    (async () => {
      try {
