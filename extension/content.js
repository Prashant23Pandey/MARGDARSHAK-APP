// MargDarshak Extension — Content Script
...
function createBadge(status) {
  ...
  badge.className = "MargDarshak-floating-badge";
  ...
  badge.setAttribute("style", [
    ...
    status === "checking" ? "animation:MargDarshak-pulse 1.2s infinite !important" : "",
  ].filter(Boolean).join(";"));
...
function createInfoCard(status, details) {
  ...
  card.className = "MargDarshak-floating-card";
  ...
  card.innerHTML = `
    ...
    <div style="display:flex;align-items:center;gap:4px;margin-top:6px;padding-top:5px;border-top:1px solid rgba(255,255,255,0.08);">
      <span style="font-size:9px;color:#5C5F73;font-weight:600;">MargDarshak</span>
    </div>
  `;
...
function scanAllImages() {
  ...
  images.forEach((img) => {
    ...
    chrome.runtime.sendMessage(
      { type: "MargDarshak_VERIFY_IMAGE", imageUrl: src },
      (response) => {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === "MargDarshak_STATUS") {
    showBadge(msg.imageUrl, msg.status, null);
  } else if (msg.type === "MargDarshak_RESULT") {
    showBadge(msg.imageUrl, msg.status || "error", msg);
  } else if (msg.type === "MargDarshak_SCAN_ALL") {
